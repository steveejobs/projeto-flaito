...
CREATE POLICY IF NOT EXISTS "signature_links_public_read"
  ON public.signature_links
  FOR SELECT
  USING (true);

-- Política: anon pode inserir (gerado no frontend via REST)
CREATE POLICY IF NOT EXISTS "signature_links_anon_insert"
  ON public.signature_links
  FOR INSERT
  WITH CHECK (true);

-- Política: anon pode atualizar status/used_at (quando o cliente assina)
CREATE POLICY IF NOT EXISTS "signature_links_anon_update"
  ON public.signature_links
  FOR UPDATE
  USING (true)
  WITH CHECK (true);
