-- Atualiza template CONTRATO para incluir assinatura do advogado
...
<div style="font-family: Arial, sans-serif; font-size: 12pt; line-height: 1.65;">
  ...
  <ul>
    <li><strong>Modalidade:</strong> {{kit.tipo_remuneracao}}</li>
    <li><strong>Percentual sobre proveito econômico/êxito:</strong> {{kit.percentual_honorarios}}%</li>
    <li><strong>Valor fixo:</strong> R$ {{kit.valor_fixo_honorarios}}</li>
    <li><strong>Forma de pagamento:</strong> {{kit.forma_pagamento}}</li>
  </ul>
  ...
  <p>
    ...
    O inadimplemento de qualquer valor ajustado acarretará a incidência
    de multa moratória de 10% (dez por cento), acrescida de juros de 1%
    (um por cento) ao mês e correção monetária, autorizando o CONTRATADO
    a suspender os serviços, independentemente de aviso prévio, sem que
    isso configure abandono de causa.
  </p>
