-- Corrige comparação de roles para ser case-insensitive (OWNER vs owner)
...
BEGIN
  ...
  -- 2. Verifica permissão do usuário (CORRIGIDO: UPPER para comparação case-insensitive)
  SELECT role INTO v_user_role
  FROM office_members
  WHERE office_id = v_office_id AND user_id = auth.uid();

  IF v_user_role IS NULL OR UPPER(v_user_role) NOT IN ('OWNER', 'ADMIN') THEN
