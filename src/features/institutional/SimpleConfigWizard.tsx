...
import { Loader2, Check, ArrowRight, ArrowLeft, Upload, Building2, UserCircle, Rocket, Image as ImageIcon } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { SignatureCanvas, SignatureCanvasApi } from "@/components/SignatureCanvas";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogDescription } from "@/components/ui/dialog";
import { PenLine, Eraser, MousePointer2, Maximize2, Minimize2, Signature, Sparkles, FileText } from "lucide-react";
...
export function SimpleConfigWizard({ officeId, onComplete }: WizardProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'signatures' | 'logos' | 'watermarks') => {
    ...
    // Redimensionar ou validar arquivo se necessário futuramente
    ...
    try {
      ...
      console.log(`[Wizard] Enviando ${type} para: ${path}`);

      // O erro de RLS geralmente acontece aqui se a migração 20260428000001 não for aplicada
      ...
      if (uploadError) {
        console.error("[Wizard] Erro Supabase Storage:", uploadError);
        throw new Error(uploadError.message === 'New row violates row level security policy for table "objects".' 
          ? "Erro de Permissão: Você precisa aplicar a migração SQL no painel do Supabase." 
          : uploadError.message);
      }
  ...
  const handleExtractedData = (data: ExtractedDocumentData) => {
    const updatedData = {
      ...profData,
      name: data.full_name || profData.name,
      // Se tiver RG no documento, tenta usar como número de registro se identNumber estiver vazio
      identNumber: profData.identNumber || data.rg || data.cpf || ""
    };
    
    setProfData(updatedData);
    setShowIAExtractionModal(false);
    
    // Verifica se podemos avançar automaticamente (tem o nome?)
    if (updatedData.name) {
      toast.success("Dados preenchidos! Avançando automaticamente...");
      setTimeout(() => {
        handleFinish(updatedData);
      }, 1500);
    } else {
      toast.success("Dados preenchidos! Por favor, confirme seu nome para continuar.");
    }
  };
  ...
  const handleFinish = async (directData?: any) => {
    ...
    try {
      ...
      if (officeId) {
        ...
        if (memberError) {
          console.error("Erro ao atualizar office_members:", memberError);
          throw new Error(`Erro no perfil do escritório: ${memberError.message}`);
        }
      }

      // 2. Salvar Branding (Logo e Marca d'água separadas)
      ...
      // 3. Salvar Profissional e Assinatura
      await mutations.updateProfessionalSettings.mutateAsync({
        name: profData.name,
        identType: profData.identType,
        identNumber: profData.identNumber,
        identUf: profData.identUf,
        signatures: [{
          role: "Profissional",
          label: profData.name,
          signatureUrl: profData.signatureUrl
        }]
      });
      ...
      await supabase.rpc('complete_onboarding_step', { p_step: 'institutional_config' });

      toast.success("Configurações salvas com sucesso!");
      setTimeout(() => onComplete(), 600);
    } catch (error: any) {
      console.error("Erro completo no handleFinish:", error);
      ...
      if (error.message?.includes("profile_professional_settings_user_id_office_id_key")) {
        errorMessage = "Você já possui uma configuração salva para este escritório. Tentamos atualizar, mas houve um conflito. Por favor, tente novamente.";
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
            <div className="flex justify-between items-center bg-blue-500/5 p-4 rounded-xl border border-blue-500/10">
              <div className="space-y-1">
                <p className="text-sm font-semibold text-white">Preenchimento Automático</p>
                <p className="text-[10px] text-slate-400">Suba seu documento (RG/CNH) para evitar digitar.</p>
              </div>
              <Button 
                variant="outline" 
                size="sm" 
                onClick={() => setShowIAExtractionModal(true)}
                className="bg-blue-600 hover:bg-blue-700 text-white border-transparent gap-2 h-9 px-4 rounded-lg shadow-lg shadow-blue-500/20 transition-all hover:scale-105"
              >
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 text-center">
            <div className="flex flex-col items-center gap-2">
              <div className="relative w-48 h-32 bg-white rounded-lg shadow-2xl flex items-center justify-center overflow-hidden border border-white/20">
                {/* Simulated Grid/Pattern for transparency reference if needed, but white paper is cleaner */}
        ...
        {step === 4 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 text-center">
            <div className="mx-auto w-full max-w-sm h-40 bg-white/5 rounded-2xl border border-white/10 relative overflow-hidden flex items-center justify-center p-6 group transition-all duration-500 hover:border-blue-500/30 hover:shadow-2xl hover:shadow-blue-500/10">
              ...
              {profData.signatureUrl ? (
                <div className="bg-white rounded-lg p-4 shadow-xl translate-y-1 transform group-hover:translate-y-0 transition-transform duration-500 relative z-10">
                  <img src={profData.signatureUrl} className="max-h-24 max-w-full object-contain" alt="Signature preview" />
                </div>
              ) : (
              ...
              <div className="flex flex-col gap-3 items-center">
                <div className="flex gap-2">
                  <label className="cursor-pointer">
                    ...
                    <Button variant="secondary" asChild className="h-12 px-6 bg-slate-800 hover:bg-slate-700 text-white font-medium rounded-xl border border-white/5 shadow-lg">
                      <span className="flex items-center gap-2">
                  ...
                  <Button 
                    ...
                    className="h-12 px-6 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl shadow-lg shadow-blue-600/20"
                  >
            ...
            <Dialog open={showSignatureModal} onOpenChange={setShowSignatureModal}>
              <DialogContent className={cn(
                ...
                isExpanded ? "w-screen h-screen max-w-none m-0 rounded-none flex flex-col" : "sm:max-w-[700px]"
              )}>
      ...
      <CardFooter className="flex justify-between border-t border-white/5 pt-6 pb-6">
        ...
        {step < 5 ? (
          <Button 
            onClick={() => setStep(s => s + 1)}
            disabled={
              (step === 1 && (!profData.name || !profData.identNumber || !profData.identUf)) ||
              (step === 2 && !brandingData.logoUrl) ||
              (step === 4 && !profData.signatureUrl) ||
              isUploading
            }
          ...
          <Button 
            ...
            disabled={!unitData.name || !unitData.address || isUploading || isSaving}
      ...
      <DialogContent className="max-w-md bg-slate-950 border-white/10 p-0 overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto minimal-scrollbar">
        <DialogHeader className="p-6 pb-2 border-b border-white/5">
          ...
          <DialogDescription className="text-slate-400 text-xs">
            Anexe seu RG ou CNH (máx. 5MB) para preencher seu perfil profissional automaticamente.
          </DialogDescription>
        </DialogHeader>
        <div className="p-6">
