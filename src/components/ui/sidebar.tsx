...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (isMobile) {
    // Note: Do not spread {...props} to Sheet as it includes ref which causes warnings
    // Sheet is a Radix Dialog and doesn't accept ref directly
    const { ref: _ignoredRef, ...sheetProps } = props as typeof props & { ref?: unknown };
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...sheetProps}>
        ...
        <SheetPrimitive.Content
          ...
          data-mobile="true"
          className={cn(
            // Full screen com fallback para browsers antigos
        ...
        >
          {/* Header fixo com botão X visível */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-sidebar-border shrink-0">
            <span className="font-semibold text-lg text-sidebar-foreground">Menu</span>
            <SheetPrimitive.Close className="rounded-md p-2 hover:bg-sidebar-accent active:bg-sidebar-accent/80 transition-colors">
              <X className="h-6 w-6" />
